...
8. **Optional Codex check on gate shape.** If useful, call `codex` per the dual-engine standard (`model: gpt-6-astra`, `sandbox: read-only`, `cwd: {repo_root}`) with the draft gates and ask whether any gate depends on a later one or can't be verified independently. Treat unavailability as in the standard. The full multi-reviewer critique comes from `plan-review` afterwards; this is a cheap sanity pass, not a substitute.
