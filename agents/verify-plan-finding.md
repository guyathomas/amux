...
- **CONFIRMED** — the claim checks out, backed by cited evidence: repo `file:line`, a quoted `prd.md` section, a docs reference, or a web source. Only CONFIRMED findings are eligible for auto-apply downstream.
...
```
{
  ...
  "evidence": [
    { "source": "repo|plan|docs|web", "ref": "src/db/schema.ts:88 (or doc/section/URL)", "note": "users table has no deleted_at column" }
  ],
