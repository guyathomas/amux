...
5. **Cross-check with Codex.** Call the `codex` MCP tool with `model: gpt-6-astra`, `sandbox: read-only`, `cwd: {repo_root}`. Give it the finding and `@` repo-relative refs, and ask it to defend the design as implemented with cited `file:line` evidence and to find holes in the proposed alternative. Treat Codex as unavailable if the call throws/times out, or the response is empty or contains error text — then verify Claude-only and set `enginesUsed: ["claude"]`.
